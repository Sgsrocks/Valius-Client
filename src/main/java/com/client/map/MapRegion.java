			...
			for (int x = 0; x < regionSizeX; x++) {
				...
				for (int z = 0; z < regionSizeY; z++) {
					...
					if ((!lowMem || (tileFlags[0][x][z] & 2) != 0
						...
						int overlayB = overlays[l][nextX][z] & 0x3FFF;
						int overlayC = overlays[l][nextX][nextZ] & 0x3FFF;
						int overlayD = overlays[l][x][nextZ] & 0x3FFF;
			...
			if (k >= 0 && k < regionSizeX && i >= 0 && i < regionSizeY) {
				...
				do {
					...
					if (l1 <= 49) {
						if(!visible)
							continue;
						overlays[l][k][i] = (short) stream.readSignedShort();
