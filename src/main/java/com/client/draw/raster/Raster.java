...
public class Raster extends Cacheable {
	...
	public static void initDrawingArea(int newHeight, int newWidth, int[] newPixels) {
		pixels = newPixels;
		width = newWidth;
		height = newHeight;
		setDrawingArea(newHeight, 0, newWidth, 0);
	}
	
	public static void drawFlippedTriangle(int x, int y, int width, int height, int colour) {
		for(int yPos = y;yPos <= y + height;yPos++) {
			drawLineBetween(x + width, yPos, x, y + height / 2, colour);
		}
	}
	public static void drawTriangle(int x, int y, int width, int height, int colour) {
		for(int yPos = y;yPos <= y + height;yPos++) {
			drawLineBetween(x, yPos, x + width, y + height / 2, colour);
		}
	}
	
	public static void drawLineBetween(int x1, int y1, int x2, int y2, int color) {
	...
	public static void drawCircle(int drawX, int drawY, int radius, int color) {
		int centerX = drawX + (radius);
		int centerY = drawY + (radius);
		double slice = 2 * Math.PI / 720;
		for(int point = 0;point<720;point++) {
			double angle = slice * point;
			int newX = (int) (centerX + (radius * Math.cos(angle)));
			int newY = (int) (centerY + (radius * Math.sin(angle)));
			int pixelPos = newX + (newY * width);
			if(pixelPos < pixels.length) {
				pixels[pixelPos] = color;
			}
		}
	...
	public static void drawRectangle(int x, int y, int width, int height, int color, boolean filled, boolean shadowed) {
		if (shadowed)
			drawRectangle(x + 1, y + 1, width, height, 0, filled, false);
		if (filled) {
			drawPixels(height - 1, y + 1, x + 1, color, width - 1);// method336
		}
		drawHorizontalLine(y, color, width - 1, x + 1);// method339
		drawHorizontalLine(y + height - 1, color, width - 1, x + 1);// method339
		method341(y + 1, color, height - 2, x + 1);// method341
		method341(y + 1, color, height - 2, x + width - 1);// method341
	...
	private static void method342(int color, int j, int alpha, int l, int i1) {
		if (j < topX || j >= bottomX)
			return;
		if (l < topY) {
			i1 -= topY - l;
	...
	public static void drawHorizontalLine2(int xPosition, int yPosition, int width, int rgbColour){
		...
		int pixelIndex = xPosition + yPosition * Raster.width;
	...
	public static void drawHorizontalAlphaGradient(int x, int y, int gradientWidth, int gradientHeight, int startColor,
			int endColor, int alpha) {
		int k1 = 0;
		int l1 = 0x10000 / gradientWidth;
		if (x < topX) {
			k1 += (topX - x) * l1;
	...
	public static void drawAlphaGradientOnSprite(Sprite sprite, int x, int y, int gradientWidth,
										  int gradientHeight, int startColor, int endColor, int alpha) {
		int k1 = 0;
		int l1 = 0x10000 / gradientHeight;
		if (x < topX) {
