package com.client.cache.graphics.font;
...
public class RSFont extends Raster {
	...
	public RSFont(boolean TypeFont, String s, Archive archive) {
		...
		stream_1.position = stream.readUShort() + 4;
						...
						if (effectString.startsWith(startImage)) {
							try {
								...
								if(iconModY < 15)
									offsetY = 15 - iconModY;
								if (transparency == 256) {
									icon.drawSprite(drawX, (drawY + (offsetY / 2)));
								} else {
									icon.drawSprite(drawX, (drawY+ (offsetY / 2)), transparency);
								}
								drawX += icon.myWidth + 1;
	...
	public void drawTransparentCharacter(int i, int i_11_, int i_12_, int i_13_, int i_14_, int i_15_, int i_16_,
		...
		if (i_13_ > 0 && i_14_ > 0) {
			createTransparentCharacterPixels(Raster.pixels, fontPixels[i], i_15_, i_20_, i_17_, i_13_, i_14_,
					i_18_, i_19_, i_16_);
		}
	}
	...
	public void drawCharacter(int character, int i_35_, int i_36_, int i_37_, int i_38_, int i_39_, boolean bool) {
		...
		if (i_37_ > 0 && i_38_ > 0) {
			createCharacterPixels(Raster.pixels, fontPixels[character], i_39_, i_43_, i_40_, i_37_, i_38_, i_41_,
					i_42_);
