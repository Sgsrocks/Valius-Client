	...
	private void readValues(Buffer stream) {
		while(true) {
			...
			if (i == 1) {
				...
				breadthScale = stream.readUShort();
			} else if (i == 5) {
				depthScale = stream.readUShort();
			} else if (i == 6) {
				orientation = stream.readUShort();
			} else if (i == 7) {
				...
				for (int k = 0; k < j; k++) {
					originalColours[k] = stream.readUShort();
					replacementColours[k] = stream.readUShort();
				}
			} else {
