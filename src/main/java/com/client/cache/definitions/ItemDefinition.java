...
case 33966:
	...
	private void readValues(Buffer stream) {
		while (true) {
			...
			if (opcode == 1)
				...
				for (int index = 0; index < size; index++) {
					originalTextureColors[index] = stream.readUShort();
					modifiedTextureColors[index] = stream.readUShort();
				}
			} else if (opcode == 65) {
				searchableItem = true;
			} else if (opcode == 78)
	...
	public Model method194(int j) {
		...
		if (l != -1) {
			...
			model = new Model(2, aclass30_sub2_sub4_sub6s, true);
		}
	...
	public Model method196(int i) {
		...
		if (k != -1)
			if (l != -1) {
				...
				model = new Model(3, aclass30_sub2_sub4_sub6_1s, true);
			} else {
