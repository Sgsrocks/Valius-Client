...
public final class IDK {
		...
		do {
			...
			if (i == 1)
				...
				for(int idx = 0;idx<length;idx++) {
					colourToFind[idx] = stream.readUShort();
					colourToReplace[idx] = stream.readUShort();
				}
			} else if (i == 41) {
				int length = stream.readUnsignedByte();
				...
				for(int idx = 0;idx<length;idx++) {
					textureToFind[idx] = stream.readUShort();
					textureToReplace[idx] = stream.readUShort();
				}
			} else if (i >= 60 && i < 70) {
	...
	public Model method540() {
		...
		Model model = new Model(j, aclass30_sub2_sub4_sub6s, true);
		if(colourToFind != null)
