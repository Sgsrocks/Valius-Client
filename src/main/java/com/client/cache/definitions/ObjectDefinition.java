...
public final class ObjectDefinition {
	...
	private Model method581(int type, int animation_id, int orientation) {
		Model model = null;
		long key;
		if (modelTypes == null) {
			if (type != 10)
			...
			if (k1 > 1)
				model = new Model(k1, aModelArray741s, true);;//fixes rotating textures on objects
		} else {
			int i1 = -1;
			...
			if (i1 == -1)
				return null;
			key = (long) ((id << 8) + (i1 << 3) + orientation) + ((long) (animation_id + 1) << 32);
		...
		Model model_3 = new Model(modifiedModelColors == null, Frame.method532(animation_id),
				orientation == 0 && animation_id == -1 && !flag && !flag2,  modifiedTexture == null, model);
	
		if (animation_id != -1) {
			model_3.method469();
			model_3.method470(animation_id);
		...
		if (modifiedTexture != null) {
			for (int index = 0; index < modifiedTexture.length; index++) {
				model_3.retexture(originalTexture[index], modifiedTexture[index]);
			}
		}
		if (flag)
		...
		if(lightPos != null) {
			model_3.light(64 + ambientLighting, 768 + lightDiffusion * 5, lightPos.x, lightPos.z, lightPos.y, !delaysShading);
		} else {
			model_3.light(64 + ambientLighting, 768 + lightDiffusion * 5, -50, -10, -50, !delaysShading);
		}
